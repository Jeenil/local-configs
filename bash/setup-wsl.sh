...
clone-work-repo() {
  ...
  if [[ ! -d "$repository_path" ]]; then
    ...
    if is-jeepatel; then
      git -C "$repository_path" config user.name "Jeenil Patel"
      git -C "$repository_path" config user.email "jpatel@logixhealth.com"
    fi
  fi
}
...
if is-jeepatel; then
  ...
  if [[ ! -s "$SSH_DIR/id_ed25519" ]]; then
    cp "/mnt/c/Users/jeepatel.CORP/.ssh/id_ed25519" "$SSH_DIR/id_ed25519"
    chmod 600 "$SSH_DIR/id_ed25519"
  fi
  if [[ ! -s "$SSH_DIR/id_ed25519.pub" ]]; then
    cp "/mnt/c/Users/jeepatel.CORP/.ssh/id_ed25519.pub" "$SSH_DIR/id_ed25519.pub"
  fi
  if [[ ! -s "$SSH_DIR/id_rsa" ]]; then
    cp "/mnt/c/Users/jeepatel.CORP/.ssh/id_rsa" "$SSH_DIR/id_rsa"
    chmod 600 "$SSH_DIR/id_rsa"
  fi
  if [[ ! -s "$SSH_DIR/id_rsa.pub" ]]; then
    cp "/mnt/c/Users/jeepatel.CORP/.ssh/id_rsa.pub" "$SSH_DIR/id_rsa.pub"
  fi

  # Write SSH config to route AzDO hosts to the RSA key.
  SSH_CONFIG="$SSH_DIR/config"
  if [[ ! -s "$SSH_CONFIG" ]]; then
    cat > "$SSH_CONFIG" << 'EOF'
Host azuredevops.logixhealth.com
    User git
    IdentityFile /home/jeepatel/.ssh/id_rsa
...
if [[ ! -d "$REPOSITORIES_DIR/configs" ]]; then
  git clone git@github.com:Jeenil/configs.git
fi

# Load Git settings.
"$REPOSITORIES_DIR/configs/bash/set-git-settings.sh"
if is-jeepatel; then
  git config --global user.name "Jeenil Patel"
  git config --global user.email "jpatel@logixhealth.com"
fi
